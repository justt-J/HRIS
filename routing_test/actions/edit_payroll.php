...
if (isset($_POST["submit"])) {
  ...
  $sql = "UPDATE `users` SET `holidays`='$holidays' , `leave`='$leave' , `gross_pay`='$gross_pay',`phil_health`='$phil_health',`sss`='$sss' , `pag_ibig`='$pag_ibig' , `health_insurance`='$health_insurance' , `total_salary`='$total_salary' , `days_worked` = '$days_worked' , `cash_advance` = '$cash_advance' WHERE id = $id";
  $mysqli = include "connection/database.php";
  $result = $mysqli->query($sql);

  if ($result) {
    header("Location: /payroll_list");
  } else {
    echo "Failed: ";
  }
}
    ...
    <div class="container d-flex justify-content-center">
      <form action="" method="post" style="width:50vw; min-width:300px;">
        ...
        <div class="mb-3">
          ...
          <label class="form-label">cash Advance!:</p></label>
          <input type="number" id = "cash_advance_input" name="cash_advance_input" value="0" >
        ...
        <div class="mb-3">
              ...
              <input type="number" id = "leave" name="leave" value="<?php echo $row['leave'] ?>">
        </div>

        <div class="mb-3">
              <label class="form-label">Holidays:</label>
              <input type="number" id = "holidays" name="holidays" value="<?php echo $row['holidays'] ?>">
        </div>
        
        
        <div class="mb-3">
              <label class="form-label">Cash Advance:</label>
              <input type="number" id = "cash_advance" name="cash_advance" value="<?php echo $row['cash_advance'] ?>">
        </div>


        <div class="mb-3">
              <label class="form-label">Days Worked:</label>
              <input type="number" id = "days_worked" name="days_worked" value="<?php echo $row['days_worked'] ?>">
        </div>
        ...
        <div>
          ...
          <a href="/payroll_list" class="btn btn-danger">Cancel</a>
        </div>
      </form>
    </div>
  </div>
  ...
  <script type="text/javascript">
        ...
        function getSalary(){
          ...
          let a = document.getElementById("days_worked");
            ...
            let cash_advance = document.getElementById("cash_advance");
            let cash_advance_input = document.getElementById("cash_advance_input");
            ...
            let total_cash_advance = cash_advance_input.valueAsNumber + cash_advance.valueAsNumber;
